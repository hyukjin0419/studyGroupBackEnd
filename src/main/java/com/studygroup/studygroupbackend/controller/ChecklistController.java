...
import com.studygroup.studygroupbackend.dto.checklistitem.update.ChecklistItemUpdateContentRequest;
import com.studygroup.studygroupbackend.dto.checklistitem.update.ChecklistItemUpdateDueDateRequest;
...
public class ChecklistController {
    ...
    @PostMapping
    public ResponseEntity<ChecklistItemCreateResponse> createChecklist(
            @RequestHeader("X-Member-Id") Long creatorId,
            @RequestBody ChecklistItemCreateRequest request) {
        return ResponseEntity.ok(checklistItemService.createChecklist(creatorId, request));
    }
    ...
    public ResponseEntity<Void> updateContent(
            ...
            @RequestBody ChecklistItemUpdateContentRequest request) {
        checklistItemService.updateContent(id, request.getContent());
    ...
    public ResponseEntity<Void> updateDueDate(
            @PathVariable Long id,
            @RequestBody ChecklistItemUpdateDueDateRequest request) {
        checklistItemService.updateDueDate(id, request.getDueDate());
        return ResponseEntity.ok().build();
    }

    @Operation(summary = "체크리스트 단일 조회 API", description = "단일 체크리스트를 조회합니다.")
    @GetMapping("/{id}")
    public ResponseEntity<ChecklistItemDetailResponse> getChecklistDetail(@PathVariable Long id) {
        return ResponseEntity.ok(checklistItemService.getChecklistDetail(id));
    }

    @Operation(summary = "체크리스트 삭제 API", description = "체크리스트를 삭제합니다.")
    @DeleteMapping("{id}")
    public ResponseEntity<ChecklistItemDeleteResponse> deleteChecklist(@PathVariable Long id) {
        return ResponseEntity.ok(checklistItemService.deleteChecklist(id));
    }
}
